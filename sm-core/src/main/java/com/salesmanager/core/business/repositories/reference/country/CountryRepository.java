...
public interface CountryRepository extends JpaRepository <Country, Integer> {
	...
	@Query("select c from Country c "
			...
			+ "where cd.language.id=?1 order by cd.name")
	List<Country> listByLanguage(Integer id);

	/** get country including zones by language **/
	@Query("select distinct c from Country c left join fetch c.descriptions cd left join fetch c.zones cz left join fetch cz.descriptions where cd.language.id=?1 order by cd.name")
